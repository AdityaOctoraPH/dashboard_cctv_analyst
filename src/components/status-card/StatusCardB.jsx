...
function make(data,new_var){
    if(data !=null ){
        // for(let i=0;i<1;i++){
            new_var.push({
                count: data[9].sum_car_down,
                title: "Car"
            },{
                count: data[9].sum_truck_s_down,
                title: "Small Truck"
            },{
                count: data[9].sum_truck_m_down,
                title: "Medium Truck"
            },{
                count: data[9].sum_truck_l_down,
                title: "Large Truck"
            },{
                count: data[9].sum_truck_xl_down,
                title: "XL Truck"
            },{
                count: data[9].sum_bus_s_down,
                title: "Small Bus"
            },{
                count: data[9].sum_bus_l_down,
                title: "Big Bus"
...
const StatusCard = props => {
    ...
    const getData = async()=> {
        axios.all([request1, request2]).then(axios.spread((...res) => {
            ...
            setstateVcratio((res[1].data.vc_ratio_down).toFixed(2));
            console.log(res)
        }))
